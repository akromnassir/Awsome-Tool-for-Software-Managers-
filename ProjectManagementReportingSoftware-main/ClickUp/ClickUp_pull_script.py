...
def return_entry_object(team, sp, prot, lst, task):
    tags = 'None' if not task.tags else '; '.join(
        [tag.name for tag in task.tags])
    priority = 'None' if task.priority is None else task.priority['priority']
    users = 'None' if task.assignees is None else '; '.join(
        [str(assignee.username) for assignee in task.assignees])
...
def generate_kva(df):
    ...
    for folder in config['folders_kva']:
        ...
        df_to_export = df[['List', 'Task', 'Sub-Task',
                           'Tags', "Priority", 'Status modified date']].copy()
        ...
        if not df_to_export.empty:
            ...
            wb.save(str(kva_nr)+"_POE_" + str(folder_name).replace(' ',
                    '')+"_"+FiscalYear+'_' + str(month)+".xlsx")
            ...
            for prot in sp.projects:  # Folder
                ...
                for lst in prot.lists:
                    if config['list'] != 'All' and lst.name != config['list']:
                        continue
                    lstTasks = lst.get_all_tasks(
                        include_closed=config['closed_tasks'], subtasks=True)
                    for counter, task in enumerate(lstTasks):
                        ...
                        if config['closed_tasks']:
                            if task.status.status == 'Closed' and task.date_updated.month == config['month'] and task.date_updated.year == config['year']:
                                entry = return_entry_object(
                                    team, sp, prot, lst, task)
                                # print(entry)
