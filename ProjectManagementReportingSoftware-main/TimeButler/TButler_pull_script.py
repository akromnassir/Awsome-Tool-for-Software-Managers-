...
# -- Description : <In South Korea, companies make use of TimeButler to keep
...
if __name__ == "__main__":
    ...
    # print(users.columns)
    # holidayentitlement
    holidayentitlement = requests.post(
        url=URL+'holidayentitlement', params=PARAMS).content
    ...
    absences = users.join(absences.set_index('User ID'),
                          on='User ID', how='inner', lsuffix='u')
    absences = absences[['First name', 'Last name', 'From', 'To', 'Half a day', 'Morning',
                         'Type', 'Extra vacation day', 'State', 'Workdays', 'Medical certificate (sick leave only)']]
    # print(absences)

    # Compute approved vacation days
    approved_absenses = absences[absences['State'].isin(['Done', 'Approved'])]
    approved_absenses_grouped = approved_absenses.groupby(
        ['First name', 'Last name'])['Workdays'].sum().reset_index()
    # print(approved_absenses_grouped)

    # Generate urlaubkonto
    konto = users.join(holidayentitlement.set_index(
        'User ID'), on='User ID', how='left')
    # print(konto.columns)
    konto = konto[['First name', 'Last name', 'Date of entry (dd/mm/yyyy)', 'User account locked', 'Vacation contingent', 'Remaining vacation',
                   'Extra vacation days', 'Additional vacation for severely challenged persons', 'Expired Vacation', 'Paid out vacation']]
