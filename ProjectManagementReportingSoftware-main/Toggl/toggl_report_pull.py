...
def gen_custom_report(content):
    ...
    fiscal_month = ['01.{:02d}.{}'.format(int(m), int(
        y)) for y, m, d in map(lambda x: x.split('-'), dates)]
    ...
    df_to_export.insert(
        4, 'tags', report_df['tags'].apply(lambda x: ', '.join(x)))
    ...
    df_to_export.insert(7, 'time_tracked_in_hours',
                        df_to_export['Duration in Hours'].apply(lambda x: round(x*4)/4))
    df_to_export = df_to_export.drop(columns=['Duration in Hours'])

    # Generate fiscal year report
    pivot_list = ['user', 'fiscal_month',
                  'project', 'task', 'tags', 'Is Billable']
    df_to_export = df_to_export.groupby(
        pivot_list)['time_tracked_in_hours'].sum().reset_index()

    # Sort by datetime decending
    df_to_export.sort_values(['user', 'fiscal_month'], ascending=[
    ...
    df_to_export.insert(2, 'ProjectName', df_to_export['project'].apply(
    ...
    df_to_export.loc[df_to_export['task'] ==
                     'CK Dynamics Project', 'KVA-Nr'] = "1"
...
def export_to_csv(content):
    content.to_csv('FY'+str(config['fiscal_year'])[:-2] +
                   '_toggl_monthly'+'.csv', index=False)  # , sep=';')


def export_to_excel(content):
    content.to_excel('FY'+str(config['fiscal_year'])[:-2] +
                     '_toggl_monthly'+'.xlsx', index=False)  # , sep=';')
...
def generate_KVA_excels():
    ...
    df_kva["Time(decimals)"] = df_kva["dur"].apply(
        lambda x: round(np.round(x/(1000*3600), 3)*4)/4)
    ...
    for x in KVA_Nums:
        ...
        df_to_export = df[['project', 'task', 'tags',
                           'Start Date', "Time(decimals)", 'dur']].copy()

        # Generate fiscal year report by Grouping on Time Columns
        pivot_list = ['project', 'task', 'tags', 'Start Date']
        df_to_export = df_to_export.groupby(
            pivot_list)[['dur', 'Time(decimals)']].sum().reset_index()
        df_to_export.rename(columns={'Start Date': 'Date'}, inplace=True)
        df_to_export["Time(h)"] = df_to_export['dur'].apply(
            lambda x: millisecondsToHHMMSS(x))
        ...
        if not df_to_export.empty:
            ...
            wb.save(str(x) + "_" + str(get_month.date())
                    [0:7]+"_POE_"+projectname+".xlsx")
...
def authenticate_and_get_workspace_content(token, user_email):
    ...
    output_list = list(Parallel(n_jobs=cores)(
        delayed(pull_data)(val, token, parameters) for val in pages_dist))
...
if __name__ == "__main__":
    ...
    try:
        workspace_content = authenticate_and_get_workspace_content(
            token, user_email)
